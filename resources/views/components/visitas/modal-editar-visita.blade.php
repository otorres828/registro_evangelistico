        ...
        <div class="bg-white overflow-auto max-w-2xl w-full max-h-full rounded shadow-lg"
            ...
            <div class="py-4 px-2">
                ...
                <div class="px-2 mb-3 last:mb-0">
                    <div class="text-xs font-semibold text-slate-400 uppercase px-2 my-3 text-center">Modificar Hora
                    </div>

                    <form action="{{ route('visitas.update', $celula) }}" method="post">
                        @csrf
                        @method('put')
                        <div class="my-2 text-sm font-semibold  uppercase mb-2">Seleccione otra fecha y hora</div>
                        <input value="{{ $fecha }}" name="fecha"
                            type="datetime-local"class="h-50 w-full rounded-lg shadow-lg" />
